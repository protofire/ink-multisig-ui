...
export function FunctionsForm() {
  ...
  const sortedAbiMessages = useMemo(() => sortAbiMessages(contract?.abi.messages), [contract?.abi.messages]);
  const [message, setMessage] = useState<AbiMessage | undefined>()
  const {argValues, setArgValues, inputData} = useArgValues(message, contract?.abi.registry);
  ...
  useEffect(() => {
    ...
    const newMessage = contract?.abi.messages.find((_message) => _message.identifier === selectValue)
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
      ...
      <InputFileDropzone label='Drop metadata file or click to select it'
        ...
        <FormControl fullWidth>
          ...
          >
            {sortedAbiMessages.map((message) => {
              return <MenuItem key={message.identifier} value={message.identifier}>
                <FunctionSignature item={message} registry={contract.abi.registry} />
              </MenuItem>
            })}
